...
if (!q) return reply("_*PLEASE WRITE THE GROUP LINK 🖇️*_")
 let result = args[0].split('https://chat.whatsapp.com/')[1]
 await conn.groupAcceptInvite(result)
     await conn.sendMessage(from , { text: `✔️ *_SUCCESSFULLY JOINED_*`}, { quoted: mek } )
} catch (e) {
await conn.sendMessage(from, { react: { text: '❌', key: mek.key } })
console.log(e)
reply(`❌ *_ERROR ACCURATED !!_*\n\n${e}`)
}
} )
...
await conn.groupRevokeInvite(from)
 await conn.sendMessage(from , { text: `*GROUP LINK RESTED* ⛔`}, { quoted: mek } )
} catch (e) {
await conn.sendMessage(from, { react: { text: '❌', key: mek.key } })
console.log(e)
reply(`❌ *ERROR ACCURATED !!*\n\n${e}`)
...
if (!isBotAdmins) return reply(msr.give_adm)   
	...
	if (!users) return reply("_*COULDN'T FIND ANY USER IN CONTEXT*_ ❌")
		...
		if  ( groupAdmins.includes(users)) return reply('❗ _*USER ALREADY AN ADMIN*_  ✔️')
		    await conn.groupParticipantsUpdate(from, [users], "promote")
			await conn.sendMessage(from,{text:`*_USER PROMOTED AS AN ADMIN_*  ✔️`},{quoted:mek })
...
if (!isBotAdmins) return reply(msr.give_adm)
		let users = mek.mentionedJid ? mek.mentionedJid[0] : mek.msg.contextInfo.participant || false;
			if (!users) return reply("_*COULDN'T FIND ANY USER IN CONTEXT*_ ❌")
		const groupAdmins = await getGroupAdmins(participants) 
		if  ( !groupAdmins.includes(users)) return reply('❗ _*USER ALREADY NOT AN ADMIN*_')
		    await conn.groupParticipantsUpdate(from, [users], "demote")
			await conn.sendMessage(from,{text:`*_USER NO LONGER AN ADMIN_*  ✔️`},{quoted:mek })
...
if (!isBotAdmins) return reply(msr.give_adm)
		...
		if(!q) return reply('_*PLEASE ADD A MESSAGE*_ ℹ️')
		let teks = `${q}`
...
async(conn, mek, m,{from, l, quoted, body, isCmd, command, mentionByTag , args, q, isGroup, sender, senderNumber, botNumber2, botNumber, pushname, isMe, isOwner, groupMetadata, groupName, participants, groupAdmins, isBotAdmins, isCreator ,isDev, isAdmins, reply}) => {
try{
		if ( !m.quoted ) return reply('_*PLEASE MENTION A MESSAGE*_ ℹ️')
		if(!q) return reply('_*PLEASE ADD A GROUP JID*_ ℹ️')
		//if ( q == "120363174739054837@g.us" ) { if ( !isDev ) return reply("❌ _*! YOU CAN SEND TAG MESSAGES TO OFFICIAL SUPPORT GROUP*_") }
